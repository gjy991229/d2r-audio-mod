...
每个新生成的 Mod 还会加入局内房间工具栏：`下一局` 先显示一个 4 秒自动关闭的确认条，确认后才以当前角色开始新的在线游戏，第一层提示与确认层提示保持同一高度；`创建房间` 与 `加入房间` 会在当前游戏中打开 D2R 原生表单。输入框保留正常焦点和中文输入法支持；D2RHub 自动流程以整段文本粘贴房名和密码，不逐字触发局内技能快捷键，密码缓存和回车提交仍由游戏处理；表单支持 Esc、右下关闭按钮和再次点击工具栏关闭。工具栏只追加布局节点，不复制第三方 Mod 的贴图，也不替换源 HUD 的其他内容。
