...
加工是“增加或更新所选组”，不是删除未选择组。以当前 r23 成品为来源时，来源中未选择、但清单有效的其他功能组及其未知未来组会原样保留；因此继续增加新功能组时，不需要改变现有清单结构或重做无关组。

r22 的声纹组仍使用旧的环境音首包配方；r23 将普通 Area 改为流式三连入场探针，并把 TZ 1023 明确定义为独立状态位。请从原版或当初未经加工的原始 Mod 生成一个新名称的 r23 Mod，之后的二次加工才能安全复用未变化的组。
